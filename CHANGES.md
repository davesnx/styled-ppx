...
- [FIX] Hoist `@import`, `@namespace`, and statement-form `@layer a, b;` rules to the front of `styled-ppx.generate`'s output, regardless of which module or library emitted them, since a browser only honors these rules when they precede every other rule and dependency order routinely places one after plain style rules. Drop `@charset` instead, with a warning naming the input file: the generated file always opens with its own leading comment, so `@charset` can never be the literal first bytes of the stylesheet, and the output is UTF-8 regardless (#581) (@davesnx)
...
- [BREAKING] Class names are `css-<hash(content)>` in every mode; the binding's `let` name no longer appears in the class name, so two bindings with byte-identical declarations now mint the same class and dedup to one rule. `--minify` only strips CSS whitespace and no longer affects class names. The binding name now travels as the label of the styles carrier, which becomes a triple `(className, style, label)` with a `CSS.label` accessor and a `CSS.make ~label` argument; `CSS.merge` joins labels. Styled components render the label as the element's `part` attribute in every build mode, so styles stay findable in DevTools (`[part~="Button"]`) without a class suffix. The `--dev` flag and the `cx-<binding>` marker class are removed; `styles=` on lowercase elements requires server-reason-react at or after 8539f79, which expands the carrier through `CSS.className`/`CSS.styles` (@davesnx)
- [FEATURE] Add PPX flag `--env development|production`. `development` keeps readable CSS (the default); `production` enables `--minify` (@davesnx)
