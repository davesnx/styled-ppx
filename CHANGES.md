...
- [BREAKING] Class names are `css-<hash(content)>` in every mode; the binding's `let` name no longer appears in the class name, so two bindings with byte-identical declarations now mint the same class and dedup to one rule. `--minify` only strips CSS whitespace and no longer affects class names. Dev-mode `label:<binding>` marker classes are now on by default; `--minify` and `--env production` turn them off, and `--dev` forces them back on regardless (@davesnx)
