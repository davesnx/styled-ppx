...
- [BREAKING] An atom whose declaration carries a `$(...)` value interpolation now mints a `in-<hash>` class instead of `a-<hash>` (same murmur2 digest, only the prefix changes; the `var(--...)` custom-property name this atom sets is unaffected). This is the first step of the atom-slot-keys work (see `.workplace/plans/atom-slot-keys_PLAN.md`): a future `CSS.merge` needs to recognize an interpolation-bundled atom - several different declarations from one binding legitimately sharing one class, by design - and never drop it or let it drop another atom, and `styled-ppx.generate`'s atom-class collision check (below) needs to skip it, since two different bundle bodies sharing a class is not a collision (@davesnx)
