...
- [FIX] Fix `Css_autoprefixer` entries that emitted a vendor-prefixed property or value no browser ever read, or that read the value with different syntax than the modern, unprefixed one: `-webkit-mask-mode` and `-webkit-mask-composite` never existed (WebKit's real `-webkit-mask-composite` uses an unrelated keyword set, so a value copy would be silently wrong rather than silently ignored); `-webkit-min-content` never existed (old WebKit's alias was the unrelated `min-intrinsic` keyword); `-ms-appearance` and `-ms-text-size-adjust` never existed; `margin-inline-start`/`-end` and `padding-inline-start`/`-end` now prefix to the real legacy names `-webkit-margin-start`/`-end` and `-webkit-padding-start`/`-end` instead of the never-shipped `-webkit-margin-inline-start`/`-end` and `-webkit-padding-inline-start`/`-end`; `min-width: fill-available`'s Firefox variant is now the real `-moz-available` instead of `-moz-fill-available`. Stop emitting `-ms-grid-row`/`-ms-grid-column` (IE's old grid module only accepted a single line number, not the modern `<start> / <end>` syntax) and `-ms-scroll-snap-type`/old `-webkit-scroll-snap-type` (both predate the axis keyword and only accepted a bare strictness keyword) and `-ms-writing-mode` (its value set is `lr-tb`/`tb-rl`/... instead of `horizontal-tb`/`vertical-rl`/...), since none of these can carry the modern value through correctly and the browsers that read them are gone. Also drop the `column` entry, a typo that matched no real CSS property (@davesnx)
