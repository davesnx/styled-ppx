...
- [FIX] Trim trailing whitespace from a declaration's value before rendering it, so a declaration terminated by `}` or end of input with a source-level space before the terminator (`{ display: flex }`) renders and hashes identically to the same declaration terminated by `;` (`{ display: flex; }`). Previously the two rendered as `display:flex;` and `display:flex ;`, minting distinct classes for byte-identical CSS (@davesnx)
- [FEATURE] Mint a build-independent **identity class** (`cid-<hash>`) for every named `[%css]` binding and `[%styled.<tag>]` component, derived from the module, submodule scope, and binding name (never a physical path or dune library name), plus an occurrence index when a name repeats within one compilation unit. Add PPX flag `--namespace <string>`, mixed into the identity hash so two libraries whose modules share a basename and binding name can be told apart; the aggregator reports an identity collision, naming both input files, the longident, and the identity, when two different bindings share an identity but not their CSS (@davesnx)
...
- [BREAKING] Class names are `css-<hash(content)>` in every mode; the binding's `let` name no longer appears in the class name, so two bindings with byte-identical declarations now mint the same class and dedup to one rule. `--minify` only strips CSS whitespace and no longer affects class names. Dev-mode `cx-<binding>` marker classes are now on by default; `--minify` and `--env production` turn them off, and `--dev` forces them back on regardless (@davesnx)
