...
- [FEATURE] Register the CSS Gaps Module Level 1 `row-rule`/`column-rule`/`rule` family (37 properties, Chrome 149+ per MDN `browser-compat-data`), superseding css-multicol-1's plain-scalar `column-rule`/`column-rule-color`/`-style`/`-width` with comma lists plus `repeat()`/`repeat(auto, ...)` (`<line-color-list>`/`<line-style-list>`/`<line-width-list>` and their `<gap-rule-list>` counterpart for the shorthand itself, mirroring `grid-template-columns`'s own track-list/`repeat()` grammar). New: `row-rule` and its own `-color`/`-style`/`-width`/`-break`/`-visibility-items`; the gap-decoration `-inset` tree on both sides (`column-rule-inset-cap-start`/`-cap-end`/`-junction-start`/`-junction-end` leaves, the `-cap`/`-junction` 1-or-2-value shorthands, the universal `-inset` cap[/junction] shorthand, and `-inset-end` on the column side, `-inset-start`/`-inset-end` on the row side, since `column-rule-inset-start` itself has no browser implementation yet); `rule-overlap`; and the bidirectional `rule`/`rule-break`/`rule-color`/`rule-style`/`rule-width`/`rule-visibility-items`/`rule-inset*` shorthands that set both sides at once. The 140 remaining standards-track-but-unimplemented and 13 dead properties (11 CSS 2.1 aural, plus `background-repeat-x`/`-y`, implemented and then removed from every browser) are still recorded for a later decision, not added here (@davesnx)
- [FEATURE] Register 7 CSS properties that the W3C property list has and css-grammar did not, each implemented in at least one major browser per MDN `browser-compat-data`: `border-shape` (`'none' | [ <basic-shape> <geometry-box>? ]{1,2}`, css-borders-4, Chrome/Edge), `flow-tolerance` (`'normal' | <length-percentage> | 'infinite'`, css-grid-3, Safari), `frame-sizing` (`'auto' | 'content-width' | 'content-height' | 'content-block-size' | 'content-inline-size'`, css-sizing-4, Chrome/Edge), `scroll-axis-lock` (`'auto' | 'none'`, css-overflow-5, Chrome/Edge), `view-transition-group` (`'normal' | 'contain' | 'nearest' | <custom-ident>`, css-view-transitions-2, Chrome/Edge), `view-transition-scope` (`'none' | 'all'`, css-view-transitions-2, Chrome/Edge), and `window-drag` (`'none' | 'move'`, css-ui-4, Chrome/Edge) (@davesnx)
- [BREAKING] An atom whose declaration carries a `$(...)` value interpolation now mints a `in-<hash>` class instead of `a-<hash>` (same murmur2 digest, only the prefix changes; the `var(--...)` custom-property name this atom sets is unaffected). `CSS.merge` (below) recognizes an interpolation-bundled atom this way - several different declarations from one binding legitimately sharing one class, by design - and never drops it or lets it drop another atom, and `styled-ppx.generate`'s atom-class collision check (below) skips it, since two different bundle bodies sharing a class is not a collision (@davesnx)
