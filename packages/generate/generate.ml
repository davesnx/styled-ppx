module Rule_set = Set.Make (String)

let of_structure ~verbose structure stylesheet =
  ...
  List.fold_left
    (fun stylesheet item ->
      ...
      | [%stri [@@@css [%e? value]]] ->
        ...
        | Some v ->
          if verbose then Printf.eprintf "Adding %s\n" v;
          Rule_set.add v stylesheet
        | None -> stylesheet)
      | _ -> stylesheet)
...
let of_pp_ml ~verbose ~filename stylesheet =
  ...
  | Ok t ->
    ...
    | Impl structure -> of_structure ~verbose structure stylesheet
    | Intf _ -> stylesheet)
...
let of_ml ~verbose ~filename stylesheet =
  try
    ...
    of_structure ~verbose structure stylesheet
  with
  | Sys_error msg ->
    if verbose then Printf.eprintf "Error reading file %s: %s\n" filename msg;
    stylesheet
...
let parse_args args =
  ...
  in
  parse [] ~output_file:None ~verbose:false ~minify:false
...
let () =
  ...
  Rule_set.iter
    (fun rule ->
      Stdlib.output_string out_channel rule;
      Stdlib.output_string out_channel separator)
    stylesheet;
