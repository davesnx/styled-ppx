...
let murmur2 (str : string) =
  print_endline (Printf.sprintf "str: '%s'" str);
  ...
  while !len >= 4 do
    ...
    (* k =
       (k & 0xffff) * 0x5bd1e995 + (((k >>> 16) * 0xe995) << 16) *)
    print_endline (Printf.sprintf "k %ld" (!k));
    print_endline (Printf.sprintf "k [%i]: %ld" !index (0x5bd1e995l ++ 1684300144l));
