...
type kind =
  | Value(string) /* CSS Value types: color, length, angle, etc. - the primitive vocabulary */
  | Property(string) /* CSS Properties: width, flex-basis, etc. - compose values */
  | Media_query(string) /* Media query rules */
  | Function(string); /* CSS Functions: rgb(), calc(), etc. */
...
/* CSS Values - reusable type vocabulary */
/* See: https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Values_and_Units */
let value = (type a, name: string, rule: Rule.rule(a)): (module RULE) => {
  (module
   {
     type result = a;
     let rule = rule;
