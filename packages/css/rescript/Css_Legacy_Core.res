...
let rec ruleToDict = (dict, rule) => {
  ...
  | PseudoClass(name, ruleset) => Js.Dict.set(dict, `:` ++ name, toJson(ruleset))
...
and toJson = rules => Std.List.reduce(rules, Js.Dict.empty(), ruleToDict)->Js.Json.object_
...
module Make = (CssImpl: Css_Core.CssImplementationIntf): (
  ...
  let keyframes = frames => CssImpl.makeKeyframes(List.fold_left(addStop, Js.Dict.empty(), frames))
...
module Converter = {
  let string_of_stops = stops =>
    stops
    ->Std.List.map(((c, l)) => Color.toString(c) ++ (` ` ++ Length.toString(l)))
...
let filter = x => D(`filter`, x->Std.List.map(Filter.toString)->Std.List.joinWith(~sep=` `))
...
let boxShadows = x => D(`boxShadow`, x->Std.List.map(Shadow.toString)->Std.List.joinWith(~sep=`, `))
