...
and toJson = rules => Std.Array.reduce(rules, Js.Dict.empty(), ruleToDict)->Js.Json.object_
...
module Make = (CssImpl: Css_Core.CssImplementationIntf): (
  ...
  let framesToDict = frames =>
    Std.Array.reduce(frames, Js.Dict.empty(), (dict, (stop, rules)) => {
...
let join = (strings, separator) =>
  Std.Array.reduceWithIndex(strings, ``, (acc, item, index) =>
    if index == 0 {
      item
    } else {
      acc ++ (separator ++ item)
...
let backgroundPosition4 = (~x, ~offsetX, ~y, ~offsetY) => D(
  ...
  (` ` ++
  (string_of_backgroundPosition(y) ++ (` ` ++ Length.toString(offsetY)))))),
)
...
let gridColumn = (start, end') => D(
  `gridColumn`,
  Std.Int.toString(start) ++ (` / ` ++ Std.Int.toString(end')),
)
...
let textEmphasisStyles = (x, y) => D(
  `textEmphasisStyles`,
  switch x {
  | #...TextEmphasisStyle.FilledOrOpen.t as fo => TextEmphasisStyle.FilledOrOpen.toString(fo)
  | #...Var.t as va => Var.toString(va)
  | #...Cascading.t as c =>
    Cascading.toString(c) ++
    (` ` ++
    switch y {
    | #...TextEmphasisStyle.Shape.t as shape => TextEmphasisStyle.Shape.toString(shape)
    | #...Var.t as va => Var.toString(va)
    | #...Cascading.t as c => Cascading.toString(c)
    })
  },
...
let textEmphasisPosition' = x =>
  switch x {
  | #...TextEmphasisPosition.OverOrUnder.t as ou => TextEmphasisPosition.OverOrUnder.toString(ou)
  | #...Var.t as va => Var.toString(va)
  | #...Cascading.t as c => Cascading.toString(c)
  }

let textEmphasisPosition = x => D(`textEmphasisPosition`, textEmphasisPosition'(x))

let textEmphasisPositions = (x, y) => D(
...
let textDecorationSkipInset = x => D(
  `textDecorationSkipInset`,
  switch x {
  | #...TextDecorationSkipInset.t as tdsi => TextDecorationSkipInset.toString(tdsi)
  | #...Var.t as va => Var.toString(va)
  | #...Cascading.t as c => Cascading.toString(c)
  },
)
...
type gridLength = [
  | trackLength
  | #repeat(RepeatValue.t, array<trackLength>)
]
...
let rec gridLengthToJs = x =>
  ...
  | #repeat(n, x) =>
    `repeat(` ++ (RepeatValue.toString(n) ++ (`, ` ++ (string_of_dimensions(x) ++ `)`)))
  | #minmax(a, b) => `minmax(` ++ (string_of_minmax(a) ++ (`,` ++ (string_of_minmax(b) ++ `)`)))
  }
...
type filter = [
  | #blur(Length.t)
  | #brightness([#percent(float) | #num(float)])
  | #contrast([#percent(float) | #num(float)])
  | #dropShadow(Length.t, Length.t, Length.t, [Color.t | Var.t])
  | #grayscale([#percent(float) | #num(float)])
  | #hueRotate(angle)
  | #invert([#percent(float) | #num(float)])
  | #opacity([#percent(float) | #num(float)])
  | #saturate([#percent(float) | #num(float)])
  | #sepia([#percent(float) | #num(float)])
  | #url(string)
  | #none
  | Var.t
  | Cascading.t
...
let filter = x => D(`filter`, x->Std.Array.map(string_of_filter)->Std.Array.joinWith(~sep=` `))
...
module Shadow = {
  ...
  let box = (~x=zero, ~y=zero, ~blur=zero, ~spread=zero, ~inset=false, color) =>
    #shadow(
      ...
      (` ` ++
      (string_of_color(color) ++ if inset {
        ` inset`
      } else {
        ``
      })))))))),
...
let fontFace = (
  ...
  let fontStyle = switch fontStyle {
  ...
  (src ++ `;`) ++
  (fontStyle ++
  (fontWeight ++ (fontDisplay ++ (sizeAdjust ++ `}`))))))
}
...
module Transition = {
  ...
  let shorthand = (~duration=#ms(0), ~delay=#ms(0), ~timingFunction=#ease, property): t =>
    #value(
      Time.toString(duration) ++
      (` ` ++
      (TimingFunction.toString(timingFunction) ++
      (` ` ++
      (Time.toString(delay) ++ (` ` ++ property))))),
    )
...
let fontSynthesisSmallCaps = x => D(
  `fontSynthesisSmallCaps`,
  switch x {
  | #...FontSynthesisSmallCaps.t as fssc => FontSynthesisSmallCaps.toString(fssc)
  | #...Var.t as var => Var.toString(var)
  | #...Cascading.t as c => Cascading.toString(c)
  },
)

let fontSynthesisPosition = x => D(
  `fontSynthesisWeight`,
  switch x {
