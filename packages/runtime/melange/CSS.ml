...
(* The carrier is `(className, inline vars, debug label)`. The label is
   ...
   nothing in prod bundles. Always go through the accessors below — the
...
let inline_vars vars =
  ...
  in
  loop (ReactDOM.Style.make ()) vars

(* `make` deliberately has no optional label argument: melange saturates
   optionals with `undefined` at call sites, which would cost bytes on
...
let merge (styles1 : styles) (styles2 : styles) =
  let merged_className =
    Js.String.trim (className styles1 ^ " " ^ className styles2)
  in
