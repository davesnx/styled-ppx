...
let prop = (type a, name: string, rule: Rule.rule(a)): (module RULE) => {
  (module
   {
     type result = a;
     let rule = rule;
...
let mq = (type a, name: string, rule: Rule.rule(a)): (module RULE) => {
  (module
   {
     type result = a;
     let rule = rule;
...
let abs = (type a, name: string, rule: Rule.rule(a)): (module RULE) => {
  (module
   {
     type result = a;
     let rule = rule;
...
let fn = (type a, name: string, rule: Rule.rule(a)): (module RULE) => {
  (module
   {
     type result = a;
     let rule = rule;
...
let packed_rules: list(module RULE) = [
  ...
  abs("-non-standard-overflow", _non_standard_overflow),
  abs("-non-standard-width", _non_standard_width),
  abs("-webkit-gradient-color-stop", _webkit_gradient_color_stop),
  abs("-webkit-gradient-point", _webkit_gradient_point),
  abs("-webkit-gradient-radius", _webkit_gradient_radius),
  abs("-webkit-gradient-type", _webkit_gradient_type),
  abs("-webkit-mask-box-repeat", _webkit_mask_box_repeat),
  abs("-webkit-mask-clip-style", _webkit_mask_clip_style),
  abs("absolute-size", absolute_size),
  abs("attr-name", attr_name),
  abs("attr-type", attr_type),
  abs("attr-unit", attr_unit),
  abs("syntax", syntax),
  abs("syntax-combinator", syntax_combinator),
  abs("syntax-component", syntax_component),
  abs("syntax-multiplier", syntax_multiplier),
  abs("syntax-single-component", syntax_single_component),
  abs("syntax-string", syntax_string),
  abs("syntax-type-name", syntax_type_name),
  abs("age", age),
  abs("alpha-value", alpha_value),
  abs("angular-color-hint", angular_color_hint),
  abs("angular-color-stop", angular_color_stop),
  abs("angular-color-stop-list", angular_color_stop_list),
  abs("hue-interpolation-method", hue_interpolation_method),
  abs("polar-color-space", polar_color_space),
  abs("rectangular-color-space", rectangular_color_space),
  abs("color-interpolation-method", color_interpolation_method),
  abs("animateable-feature", animateable_feature),
  abs("attachment", attachment),
  abs("attr-fallback", attr_fallback),
  abs("attr-matcher", attr_matcher),
  abs("attr-modifier", attr_modifier),
  abs("attr-name", attr_name),
  abs("attribute-selector", attribute_selector),
  abs("auto-repeat", auto_repeat),
  abs("auto-track-list", auto_track_list),
  abs("baseline-position", baseline_position),
  abs("basic-shape", basic_shape),
  abs("bg-image", bg_image),
  abs("bg-layer", bg_layer),
  abs("bg-position", bg_position),
  abs("bg-size", bg_size),
  abs("blend-mode", blend_mode),
  abs("border-radius", border_radius),
  abs("bottom", bottom),
  abs("box", box),
  abs("calc-product", calc_product),
  abs("calc-sum", calc_sum),
  abs("calc-value", calc_value),
  abs("cf-final-image", cf_final_image),
  abs("cf-mixing-image", cf_mixing_image),
  abs("class-selector", class_selector),
  abs("clip-source", clip_source),
  abs("color", color),
  abs("color-stop", color_stop),
  abs("color-stop-angle", color_stop_angle),
  abs("color-stop-length", color_stop_length),
  abs("color-stop-list", color_stop_list),
  abs("combinator", combinator),
  abs("common-lig-values", common_lig_values),
  abs("compat-auto", compat_auto),
  abs("complex-selector", complex_selector),
  abs("complex-selector-list", complex_selector_list),
  abs("composite-style", composite_style),
  abs("compositing-operator", compositing_operator),
  abs("compound-selector", compound_selector),
  abs("compound-selector-list", compound_selector_list),
  abs("content-distribution", content_distribution),
  abs("content-list", content_list),
  abs("content-position", content_position),
  abs("content-replacement", content_replacement),
  abs("contextual-alt-values", contextual_alt_values),
  abs("counter-style", counter_style),
  abs("counter-style-name", counter_style_name),
  abs("cubic-bezier-timing-function", cubic_bezier_timing_function),
  abs("declaration", declaration),
  abs("declaration-list", declaration_list),
  abs("deprecated-system-color", deprecated_system_color),
  abs("discretionary-lig-values", discretionary_lig_values),
  abs("display-box", display_box),
  abs("display-inside", display_inside),
  abs("display-internal", display_internal),
  abs("display-legacy", display_legacy),
  abs("display-listitem", display_listitem),
  abs("display-outside", display_outside),
  abs("east-asian-variant-values", east_asian_variant_values),
  abs("east-asian-width-values", east_asian_width_values),
  abs("ending-shape", ending_shape),
  abs("explicit-track-list", explicit_track_list),
  abs("family-name", family_name),
  abs("feature-tag-value", feature_tag_value),
  abs("feature-type", feature_type),
  ...
  abs("filter-function-list", filter_function_list),
  abs("final-bg-layer", final_bg_layer),
  abs("fixed-breadth", fixed_breadth),
  abs("fixed-repeat", fixed_repeat),
  abs("fixed-size", fixed_size),
  abs("font-stretch-absolute", font_stretch_absolute),
  abs("font-variant-css21", font_variant_css21),
  abs("font-weight-absolute", font_weight_absolute),
  ...
  fn("var", function_var),
  abs("gender", gender),
  abs("general-enclosed", general_enclosed),
  abs("generic-family", generic_family),
  abs("generic-name", generic_name),
  abs("generic-voice", generic_voice),
  abs("geometry-box", geometry_box),
  abs("gradient", gradient),
  abs("grid-line", grid_line),
  abs("historical-lig-values", historical_lig_values),
  abs("hue", hue),
  abs("id-selector", id_selector),
  abs("image", image),
  abs("image-set-option", image_set_option),
  abs("image-src", image_src),
  abs("image-tags", image_tags),
  abs("inflexible-breadth", inflexible_breadth),
  abs("keyframe-block", keyframe_block),
  abs("keyframe-block-list", keyframe_block_list),
  abs("keyframe-selector", keyframe_selector),
  abs("keyframes-name", keyframes_name),
  abs("leader-type", leader_type),
  abs("left", left),
  abs("line-name-list", line_name_list),
  abs("line-names", line_names),
  abs("line-style", line_style),
  abs("line-width", line_width),
  abs("linear-color-hint", linear_color_hint),
  abs("linear-color-stop", linear_color_stop),
  abs("mask-image", mask_image),
  abs("mask-layer", mask_layer),
  abs("mask-position", mask_position),
  abs("mask-reference", mask_reference),
  abs("mask-source", mask_source),
  abs("masking-mode", masking_mode),
  mq("and", media_and),
  ...
  mq("type", media_type),
  abs("mf-boolean", mf_boolean),
  abs("mf-name", mf_name),
  abs("mf-plain", mf_plain),
  abs("mf-range", mf_range),
  abs("mf-value", mf_value),
  abs("name-repeat", name_repeat),
  abs("named-color", named_color),
  abs("namespace-prefix", namespace_prefix),
  abs("ns-prefix", ns_prefix),
  abs("nth", nth),
  abs("number-one-or-greater", number_one_or_greater),
  abs("number-percentage", number_percentage),
  abs("alpha-value", number_zero_one),
  abs("numeric-figure-values", numeric_figure_values),
  abs("numeric-fraction-values", numeric_fraction_values),
  abs("numeric-spacing-values", numeric_spacing_values),
  abs("outline-radius", outline_radius),
  abs("overflow-position", overflow_position),
  abs("page-body", page_body),
  abs("page-margin-box", page_margin_box),
  abs("page-margin-box-type", page_margin_box_type),
  abs("page-selector", page_selector),
  abs("page-selector-list", page_selector_list),
  abs("paint", paint),
  abs("position", position),
  abs("positive-integer", positive_integer),
  ...
  abs("pseudo-class-selector", pseudo_class_selector),
  abs("pseudo-element-selector", pseudo_element_selector),
  abs("pseudo-page", pseudo_page),
  abs("quote", quote),
  abs("ratio", ratio),
  abs("relative-selector", relative_selector),
  abs("relative-selector-list", relative_selector_list),
  abs("relative-size", relative_size),
  abs("repeat-style", repeat_style),
  abs("right", right),
  abs("self-position", self_position),
  abs("shadow", shadow),
  abs("shadow-t", shadow_t),
  abs("shape", shape),
  abs("shape-box", shape_box),
  abs("shape-radius", shape_radius),
  abs("side-or-corner", side_or_corner),
  abs("single-animation", single_animation),
  ...
  abs("size", size),
  abs("ray-size", ray_size),
  abs("radial-size", radial_size),
  abs("step-position", step_position),
  abs("step-timing-function", step_timing_function),
  abs("subclass-selector", subclass_selector),
  abs("supports-condition", supports_condition),
  abs("supports-decl", supports_decl),
  abs("supports-feature", supports_feature),
  abs("supports-in-parens", supports_in_parens),
  abs("supports-selector-fn", supports_selector_fn),
  abs("svg-length", svg_length),
  abs("svg-writing-mode", svg_writing_mode),
  abs("symbol", symbol),
  abs("symbols-type", symbols_type),
  abs("target", target),
  abs("timing-function", timing_function),
  abs("top", top),
  abs("track-breadth", track_breadth),
  abs("track-group", track_group),
  abs("track-list", track_list),
  abs("track-list-v0", track_list_v0),
  abs("track-minmax", track_minmax),
  abs("track-repeat", track_repeat),
  abs("track-size", track_size),
  abs("transform-function", transform_function),
  abs("transform-list", transform_list),
  abs("type-or-unit", type_or_unit),
  abs("type-selector", type_selector),
  abs("viewport-length", viewport_length),
  abs("wq-name", wq_name),
  abs("x", x),
  abs("y", y),
  /* TODO: calc needs to be available in length */
  abs("extended-length", extended_length),
  abs("extended-frequency", extended_frequency),
  abs("extended-angle", extended_angle),
  abs("extended-time", extended_time),
  abs("extended-percentage", extended_percentage),
];
