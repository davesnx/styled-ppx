...
let rec ruleToDict dict rule =
  ...
  [@ns.braces ])
and toJson rules =
  (rules |. (Belt.List.reduce (Js.Dict.empty ()) ruleToDict)) |.
    Js.Json.object_
...
let backdropFilter x =
  D
    ({js|backdropFilter|js},
      ((x |. (Belt.List.map Types.BackdropFilter.toString)) |.
         (join {js|, |js})))
...
let backgroundPositions bp =
  D
    ({js|backgroundPosition|js},
      ((bp |. (Belt.List.map string_of_backgroundposition)) |.
         (join {js|, |js})))
...
let contentRules xs =
  D
    ({js|content|js},
      ((xs |. (Belt.List.map string_of_content)) |. (join {js| |js})))
...
let countersIncrement xs =
  D
    ({js|counterIncrement|js},
      ((xs |. (Belt.List.map string_of_counter_increment)) |.
         (join {js| |js})))
...
let transforms x =
  D
    ({js|transform|js},
      ((x |. (Belt.List.map Transform.toString)) |. (join {js| |js})))
...
let string_of_dimension x =
  ...
  | `minmax (a, b) ->
      {js|minmax(|js} ^
        ((string_of_minmax a) ^
           ({js|,|js} ^ ((string_of_minmax b) ^ {js|)|js})))
type nonrec minmax =
  ...
  | `repeat (n, x) ->
      {js|repeat(|js} ^
        ((RepeatValue.toString n) ^
           ({js|, |js} ^ ((string_of_dimension x) ^ {js|)|js})))
  | `minmax (a, b) ->
      {js|minmax(|js} ^
        ((string_of_minmax a) ^
           ({js|,|js} ^ ((string_of_minmax b) ^ {js|)|js})))
...
let gridArea3 s s2 s3 =
  D
    ({js|gridArea|js},
      ((GridArea.toString s) ^
         ({js| / |js} ^
            ((GridArea.toString s2) ^ ({js| / |js} ^ (GridArea.toString s3))))))
...
let filter x =
  D
    ({js|filter|js},
      ((x |. (Belt.List.map string_of_filter)) |. (join {js| |js})))
module Shadow =
  struct
...
let boxShadows x =
  D
    ({js|boxShadow|js},
      ((x |. (Belt.List.map Shadow.toString)) |. (join {js|, |js})))
        ...
        (fun w ->
           {js|font-weight: |js} ^
             ((match w with
               | #FontWeight.t as f -> FontWeight.toString f
               | #Var.t as va -> Var.toString va
               | #Cascading.t as c -> Cascading.toString c) ^ {js|;|js})) in
    let fontDisplay =
        ...
        ((string_of_time duration) ^
           ({js| |js} ^
              ((TimingFunction.toString timingFunction) ^
                 ({js| |js} ^
                    ((string_of_time delay) ^ ({js| |js} ^ property))))))
                                ...
                                ((AnimationIterationCount.toString
                                   ...
                                   ({js| |js} ^
                                      ((AnimationDirection.toString direction)
                                         ^
                                         ({js| |js} ^
                                            ((AnimationFillMode.toString
                                                fillMode)
                                               ^
                                               ({js| |js} ^
                                                  (AnimationPlayState.toString
                                                     playState)))))))))))))))
    let toString x = match x with | `value v -> v
  end[@@ns.doc "\n * Animation\n "]
...
let animations x =
  D
    ({js|animation|js},
      ((x |. (Belt.List.map Animation.toString)) |. (join {js|, |js})))
