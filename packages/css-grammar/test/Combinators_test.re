...
let tests = [
  test("A B", _ => {
    let parser = parse_exn([%spec "A B"]);
    let ((), ()) = parser("A B");
    ();
  }),
  test("A B C", _ => {
    // TODO: fix this type
    let ((), (), ()) = parse_exn([%spec "A B C"], "A B C");
    ();
  }),
  test("<number> B", () => {
    let (number, ()) = parse_exn([%spec "<number> B"], "15 B");
    Alcotest.check(~pos=__POS__, Alcotest.float(1.), "", number, 15.0);
  }),
  /* test{
       let parser = parse_exn([%spec "<number> B"]);
  ...
  test("A | B | C", _ => {
    let parser = parse_exn([%spec "A | B | C"]);
    let () =
      switch (parser("A")) {
      | `A => ()
      | `B
  ...
  test("A | [A B]", _ => {
    let parser = parse_exn([%spec "A | [A B]"]);
    let () =
      switch (parser("A B")) {
      | `Static(_) => ()
  ...
  test("[ A? [B | C] ]+ A?", _ => {
    let parser = parse_exn([%spec "[ A? [B | C] ]+ A?"]);
  ...
  test("xor with three alternatives", _ => {
    let parser = parse_exn([%spec "red | blue | green"]);
    switch (parser("red")) {
    | `Red => ()
  ...
  test("xor with data types", () => {
    let parser = parse_exn([%spec "<number> | <percentage> | auto"]);
    switch (parser("50")) {
    | `Number(n) =>
  ...
  test("xor with complex combinations", _ => {
    let parser = parse_exn([%spec "[A B] | [C D] | E"]);
    switch (parser("A B")) {
    | `Static_0((), ()) => ()
  ...
  test("xor with mixed types and keywords", () => {
    let parser = parse_exn([%spec "<number> | inherit | initial"]);
    switch (parser("42")) {
    | `Number(n) =>
