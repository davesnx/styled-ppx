...
let keyword kw =
  match kw with
  | "<=" -> Rule.Pattern.expect (Tokens.DELIM "<=")
  | ">=" -> Rule.Pattern.expect (Tokens.DELIM ">=")
  | _ ->
    let kw_lower = String.lowercase_ascii kw in
    Rule.Pattern.token (function
      | Tokens.IDENT s when String.lowercase_ascii s = kw_lower -> Ok ()
      | Tokens.TAG s when String.lowercase_ascii s = kw_lower -> Ok ()
      | token ->
        Error
          [
            "Expected '"
...
let comma = Rule.Pattern.expect Tokens.COMMA

let delim = function
  | "(" -> Rule.Pattern.expect Tokens.LEFT_PAREN
  | ")" -> Rule.Pattern.expect Tokens.RIGHT_PAREN
  | "[" -> Rule.Pattern.expect Tokens.LEFT_BRACKET
  | "]" -> Rule.Pattern.expect Tokens.RIGHT_BRACKET
  | ":" -> Rule.Pattern.expect Tokens.COLON
  | ";" -> Rule.Pattern.expect Tokens.SEMI_COLON
  | "*" -> Rule.Pattern.expect Tokens.ASTERISK
  | "." -> Rule.Pattern.expect Tokens.DOT
  | "," -> Rule.Pattern.expect Tokens.COMMA
  | "+" ->
    Combinators.xor
      [
        Rule.Pattern.expect (Tokens.COMBINATOR "+");
        Rule.Pattern.expect (Tokens.DELIM "+");
        Rule.Pattern.expect (Tokens.OPERATOR "+");
      ]
  | "-" ->
    Combinators.xor
      [
  ...
  | s -> Rule.Pattern.expect (Tokens.DELIM s)

let function_call name rule =
  Rule.Match.bind
    (Rule.Pattern.token (function
      | Tokens.FUNCTION called_name when name = called_name -> Ok ()
      | token ->
        Error
          [
            "Expected 'function "
    ...
    (fun () ->
      Rule.Match.bind rule (fun value ->
        Rule.Match.bind (Rule.Pattern.expect Tokens.RIGHT_PAREN) (fun () ->
          Rule.Match.return value)))

let interpolation : string list Rule.rule =
  Rule.Pattern.token (function
    | Tokens.INTERPOLATION parts -> Ok parts
    | _ -> Error [ "Expected interpolation." ])

let number_pure : float Rule.rule =
  Rule.Pattern.token (function
    | Tokens.NUMBER num_str -> Ok (Float.of_string num_str)
    | token ->
      Error
...
let percentage_pure : float Rule.rule =
  Rule.Match.bind number_pure (fun num ->
    Rule.Match.bind (Rule.Pattern.expect Tokens.PERCENT) (fun () ->
      Rule.Match.return num))

let length_unit : length_unit Rule.rule =
  Rule.Pattern.token (function
    | Tokens.FLOAT_DIMENSION (number_str, dimension)
    | Tokens.DIMENSION (number_str, dimension) ->
...
let function_clamp : (calc_sum * calc_sum * calc_sum) Rule.rule =
  function_call "clamp"
    (Rule.Match.bind calc_sum (fun min ->
       Rule.Match.bind (Rule.Pattern.expect Tokens.COMMA) (fun () ->
...
let custom_ident : string Rule.rule =
  Rule.Pattern.token (function
    | Tokens.IDENT s -> Ok s
    | Tokens.TAG s -> Ok s
    | Tokens.STRING s -> Ok s
    | _ -> Error [ "Expected an identifier." ])

let dashed_ident : string Rule.rule =
  Rule.Pattern.token (function
    | Tokens.IDENT s when String.length s > 2 && String.sub s 0 2 = "--" -> Ok s
...
let hex_color : string Rule.rule =
  Rule.Pattern.token (function
    | Tokens.HASH str when String.length str >= 3 && String.length str <= 8 ->
      Ok str
