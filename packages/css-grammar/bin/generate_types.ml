...
let generate_witness_code (registry_entries : (string * string) list) : string =
  if registry_entries = [] then begin
    ...
    let registry_key css_name type_name =
      if String.length type_name > 9 && String.sub type_name 0 9 = "property_"
      then "property_" ^ css_name
      else css_name
    in
    ...
    Buffer.add_string buf
      ...
      \    Properties are prefixed with 'property_' to avoid collisions with \
       values. *)\n";
    Buffer.add_string buf
    ...
    Buffer.add_string buf
      ...
      \    Uses physical equality for O(1) performance instead of O(n) pattern \
       matching.\n\
      \    This is safe because GADT constructors have unique runtime \
       representations. *)\n";
    Buffer.add_string buf
...
let () =
  ...
  let write_output out_channel =
    Printf.fprintf out_channel
      ...
      \    The witness system enables type-safe lookups in the CSS rule \
       registry.\n\
      ...
      \      lookup Types.W_property_color  (* Returns property_color \
       Rule.rule *)\n\
      \ *)\n\n";
