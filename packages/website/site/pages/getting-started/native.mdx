...
- `styled-ppx.css_native` is the library with the CSS bindings
- `styled-ppx.emotion_native` is the native library which implementaits `emotion.sh` on the server. Capable of storing CSS, hashing it and generating a unique classnames, autoprefixing, etc.
