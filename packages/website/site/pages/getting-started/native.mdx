...
- `CssJs.get_stylesheet` returns a string with all styles
- `CssJs.style_tag` returns a `<style />` React element, with all styles. This is designed to be used with server-reason-react.
...
let stylesheet: string = CssJs.get_stylesheet();
