...
`%styled.{{tag}}{:rescript}` `[%styled.{{tag}}]{:reason}` and `%cx{:rescript}` `[%cx]{:reason}` support an alternative to the string-based API. It's the **Array API**, where the rules can be defined as an array of `Css.rules{:rescript}` `Css.rules{:reason}`.

A `Css.rule{:rescript}` `Css.rule{:reason}` can be created with [`%css`](./css) or [`CssJs`](./runtime) calls.
