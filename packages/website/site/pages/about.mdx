...
- The need for learning a new DSL on top of CSS was tedious. Very fancy for simple properties, but almost impossible for more complex ones (a classic example `width: calc(100% - 20px){:styled-ppx-css}` became `CssJs.width(calc(min, percent(100.), px(20))){:rescript}` `CssJs.width(calc(min, percent(100.), px(20)));{:reason}`). In real world usage I would end up using `CssJs.unsafe`.
