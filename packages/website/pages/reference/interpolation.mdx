...
If you aren't familiar with `%css{:rescript}` extension, take a look [in the `%css` section](./css.md)
...
The workaround is to use the [Array API](./array.md) to generate `%cx{:rescript}` calls, like this example:
