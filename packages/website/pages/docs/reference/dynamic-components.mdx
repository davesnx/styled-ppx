...
All rules from [Interpolation](./interpolation.md) are applied here. In the example, `color` and `background` are `CssJs.Color.t` since it gets inferred from the CSS property. It's called type holes https://twitter.com/davesnx/status/1552305210558742528
