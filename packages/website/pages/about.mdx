...
- The need for learning a new DSL on top of CSS was tedious. Very fancy for simple properties, but almost impossible for more complex ones (a classic example `width: calc(100% - 20px)` became `CssJs.width('calc('min, percent(100.), 'px(20)));`). For the real world usage I endup using `CssJs.property("property", "value")` most of the times.
