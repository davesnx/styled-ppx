...
- The need for learning a new DSL on top of CSS was tedious. Very fancy for simple properties, but almost impossible for more complex ones (a classic example `width: calc(100% - 20px){:styled-ppx-css}` became `CSS.width(calc(min, percent(100.), px(20)));{:reason}`). In real world usage I would end up using `CSS.unsafe`.
...
Using CSS enables all sorts of integrations: Editors, DevTools, prototyping tools, Github Copilot, etc. Even for designers that don't want to understand or care about <Ml>OCaml</Ml><Re>Reason</Re>.

### It can be more powerful than the JavaScript equivalents
Enabling type-safety in CSS is a nice to have, rather than a hard requirement, but nevertheless useful. styled-ppx brings an entire CSS compiler and type-checker. This enables features from SASS (like supporting future CSS version features) with the ergonomics of emotion, inlining your styles and code together.

Since 0.62.0 this vision is complete: all CSS is [statically extracted](/how-it-works) at compile time into atomic, content-addressed rules. Your styles ship as a plain css file and a minimal runtime for interpolation, something the JavaScript-based equivalents can only approximate.
