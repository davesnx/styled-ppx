...
- **Server rendering is just rendering.** Link `styles.css` in your `<head>` and render — no orchestration needed. (`[%styled.global]` renders one `<style>` element for `:root` variable *values* when you interpolate in globals.)
