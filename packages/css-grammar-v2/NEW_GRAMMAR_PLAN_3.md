...
Create a new `css-grammar-v2` package that generates OCaml types and parsers from CSS BNF specifications using a PPX. The goal is to make the system more maintainable than the current 15,000+ line `Parser.ml`.
...
let property_margin = [%spec "[ <length> | <percentage> | 'auto']{1,4}" "Css_types.Margin"]

(* Generate both the type AND the spec record *)
[%%spec_t let property_margin = [%spec "[ <length> | <percentage> | 'auto']{1,4}" "Css_types.Margin"]]
```
...
let property_margin = {
  ...
  runtime_module_path = Some "Css_types.Margin";
}
```
...
type 'a spec = {
  rule: 'a Rule.rule;
  extract_interpolations: 'a Rule.rule -> (string * string) list;
...
Interpolation should be part of each Css_data_type (aka known as Standard.*) for example Standard.length should support both the parsing rules from Standard.length + interpolation.
...
A single large generated file with all specs
...
Use OCaml's recursive type definitions (`type ... and ...`)
...
The function extract_interpolation should traverse the rules, and return the right "Css_types.XXX.toString" for each type.
...
- Keep a similar global registry
...
**Options:**
- Skip color functions for POC, just support interpolation

---

### 8. Testing Strategy

- Cram tests (like existing `packages/ppx/test/`)
...
**Options:**
- `packages/css-grammar-v2/` (completely new)
...
- Include full calc support from the start
...
1. **`Properties.ml`** - margin, padding, border, position, layout-grid-mode, background-color
