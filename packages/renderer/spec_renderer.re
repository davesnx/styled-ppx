...
let render_multiplier =
  ...
  | Repeat_by_comma(i, None) =>
    Format.sprintf("Repeat_by_comma(%i, None)", i)
  | Repeat_by_comma(i, Some(x)) =>
    Format.sprintf("Repeat_by_comma(%i, Some(%i))", i, x)
...
let rec render_value =
  ...
  | Function_call(s, v) =>
    Format.sprintf("Function_call(%s, %s)", s, render_value(v))
