...
and render_media_query = (ar: At_rule.t): Parsetree.expression => {
  ...
  let rules =
    ...
    | Rule_list(declaration) =>
      render_declarations(~loc, declaration) |> Builder.pexp_array(~loc)
    | Stylesheet(_) =>
...
and render_declaration = (~loc, d: Declaration.t) => {
  let (property, name_loc) = d.name;
  let (_valueList, _loc) = d.value;
  /* String.trim is a hack, location should be correct and not contain any whitespace */
...
and render_declarations = (~loc, (ds, _loc: Location.t)) => {
  ds |> List.concat_map(
    declaration =>
      switch (declaration) {
      | Rule.Declaration(decl) => render_declaration(~loc, decl)
...
and render_style_rule = (ident, rule: Style_rule.t): Parsetree.expression => {
  ...
  let selector_expr = render_declarations(~loc, rule.block) |> Builder.pexp_array(~loc);
  ...
  let keyframes =
    ...
    |> List.map(declaration => {
      switch (declaration) {
        ...
        let rules =
          render_declarations(~loc, block) |> Builder.pexp_array(~loc);
