  ...
  [@css ".css-fhnx1u{color:var(--var-1uix7by);}"];
  [@css ".css-k008qs{display:flex;}"];
  [@css ".css-fhnx1u{background-color:var(--id-1ks2xuv);}"];
  [@css.bindings
    [("Output.DynamicComponent", "cid-1hmg1vc", "css-fhnx1u css-k008qs")]
  ];
  module DynamicComponent = {
    ...
    external makeStylesObject:
      ...
      Js.t({..});
    [@mel.get]
    external classNameGet: makeProps(_, _) => option(string) = "className";
    [@mel.get]
    external innerRefGet: makeProps(_, _) => option(ReactDOM.domRef) =
      "innerRef";
    [@mel.get] external as_Get: makeProps(_, _) => option(string) = "as";
    [@mel.get] external idGet: makeProps(_, _) => 'id = "id";
    [@mel.get] external varGet: makeProps(_, _) => 'var = "var";
    ...
    external assign2: (Js.t({..}), makeProps(_, _), Js.t({..})) => Js.t({..}) =
      "Object.assign";
    let styles = (~var, ~id, _) =>
      CSS.make(
    ...
    let make = (props: makeProps('id, 'var)) => {
      let className =
        CSS.className(styles(~id=idGet(props), ~var=varGet(props), ()))
        ++ getOrEmpty(classNameGet(props))
      and style =
        CSS.styles(styles(~id=idGet(props), ~var=varGet(props), ()))
      and part = CSS.label(styles(~id=idGet(props), ~var=varGet(props), ()));
      let stylesObject =
        makeStylesObject(
          ~className,
          ~style,
