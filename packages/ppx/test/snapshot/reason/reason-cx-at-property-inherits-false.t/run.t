...
vars (top-level / &:hover / @media) and withheld for descendant-read vars.
  ...
  let topLevel =
    CSS.make(
      "label:topLevel cid-1lkhdb3 css-1p250wn",
      [("--color-qqxh28", CSS.Types.Color.toString(color))],
    );
  let hover =
    CSS.make(
      "label:hover cid-ng6izt css-66gnkb",
      [("--color-1g10n3y", CSS.Types.Color.toString(color))],
    );
  let media =
    CSS.make(
      "label:media cid-12d3e0w css-kesuu8",
      [("--color-7zzrwb", CSS.Types.Color.toString(color))],
    );
  let descendant =
    CSS.make(
      "label:descendant cid-1g3dzrb css-l90awb",
      [("--color-1bmfpl7", CSS.Types.Color.toString(color))],
    );
  let bundleSpan =
    CSS.make(
      "label:bundleSpan cid-ijx48y css-1s0cn0c",
      [("--color-168gc9v", CSS.Types.Color.toString(color))],
    );
  let customFeeder =
    CSS.make(
      "label:customFeeder cid-195hxjc css-1s7c43s",
      [("--str-k9lkg9", str)],
    );
  let pseudoElement =
    CSS.make(
      "label:pseudoElement cid-4p9l0t css-1uu31vv",
      [("--color-s2338j", CSS.Types.Color.toString(color))],
    );
  let legacyPseudoElement =
    CSS.make(
      "label:legacyPseudoElement cid-4uo0i7 css-drvy9e",
      [("--color-9jv993", CSS.Types.Color.toString(color))],
    );
  let mixedPseudo =
    CSS.make(
      "label:mixedPseudo cid-1rr25fw css-1ngjc2o",
      [("--color-1p3gcae", CSS.Types.Color.toString(color))],
    );
  let pseudoClassOnly =
    CSS.make(
      "label:pseudoClassOnly cid-1f0poos css-r782y4",
