...
vars (top-level / &:hover / @media) and withheld for descendant-read vars.
  ...
  let topLevel =
    CSS.make(
      "cx-topLevel cid-1lkhdb3 css-kusjgz",
      [("--color-1a279q8", CSS.Types.Color.toString(color))],
    );
  let hover =
    CSS.make(
      "cx-hover cid-ng6izt css-lfbwy0",
      [("--color-4uzv5u", CSS.Types.Color.toString(color))],
    );
  let media =
    CSS.make(
      "cx-media cid-12d3e0w css-19xpxeg",
      [("--color-km0lr0", CSS.Types.Color.toString(color))],
    );
  let descendant =
    CSS.make(
      "cx-descendant cid-1g3dzrb css-1g3cfa1",
      [("--color-ump0qt", CSS.Types.Color.toString(color))],
    );
  let bundleSpan =
    CSS.make(
      "cx-bundleSpan cid-ijx48y css-1d8si03",
      [("--color-7srqv7", CSS.Types.Color.toString(color))],
    );
  let customFeeder =
    CSS.make(
      "cx-customFeeder cid-195hxjc css-1l36lcw",
      [("--str-vye19e", str)],
    );
  let pseudoElement =
    CSS.make(
      "cx-pseudoElement cid-4p9l0t css-1vdqaxy",
      [("--color-12r6fyr", CSS.Types.Color.toString(color))],
    );
  let legacyPseudoElement =
    CSS.make(
      "cx-legacyPseudoElement cid-4uo0i7 css-yfluo3",
      [("--color-e7n16g", CSS.Types.Color.toString(color))],
    );
  let mixedPseudo =
    CSS.make(
      "cx-mixedPseudo cid-1rr25fw css-i81g8d",
      [("--color-xrtge", CSS.Types.Color.toString(color))],
    );
  let pseudoClassOnly =
    CSS.make(
      "cx-pseudoClassOnly cid-1f0poos css-11o9ny0",
