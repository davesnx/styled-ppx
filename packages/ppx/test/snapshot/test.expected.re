...
let classNameWithMultiLine = CssJs.style(. [|CssJs.label("classNameWithMultiLine"), CssJs.display(`block)|]);
...
let classNameWithCss =
  CssJs.style(. [|CssJs.label("classNameWithCss"), cssRule, CssJs.backgroundColor(CssJs.green)|]);
...
module SequenceDynamicComponent = {
  ...
  let styles = (~var) => {
    ...
    CssJs.style(. [|CssJs.unsafe("color", var), CssJs.display(`block)|]);
  };
  let make = (props: makeProps('var)) => {
    let stylesObject = {"className": styles(~var=varGet(props))};
    let newProps = Js.Obj.assign(stylesObject, Obj.magic(props));
