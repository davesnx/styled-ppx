...
||||||| parent of f1e33b2 (feat: Enable interpolation for padding)
      CssJs.borderTop(alpha),
      CssJs.marginTop(wat),
      CssJs.color(mono100),
      CssJs.width(`vw(100.)),
      CssJs.margin(`pxFloat(10.)),
      CssJs.margin2(~v=`pxFloat(10.), ~h=`pxFloat(10.)),
      CssJs.margin3(
        ~top=`pxFloat(10.),
        ~h=`pxFloat(10.),
        ~bottom=`pxFloat(10.),
...
>>>>>>> 415e329 (feat: Implement missing interp properties)
  ...
  let make = (props: makeProps) => {
    let stylesObject = {"className": styles};
    let newProps = Js.Obj.assign(stylesObject, Obj.magic(props));
    createVariadicElement("hr", newProps);
  };
};
||||||| parent of 97a5133 (test(snapshot): Remove interpolation tests)
module Hr = {
  [@bs.deriving abstract]
  type makeProps = {
    [@bs.optional]
...
=======
>>>>>>> 97a5133 (test(snapshot): Remove interpolation tests)
