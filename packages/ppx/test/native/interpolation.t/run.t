Interpolation native PPX transformations are checked as a cram snapshot.
  ...
  [@css ".a-4enh4f{color:var(--mono100-mn2jt5);}"];
  [@css ".a-7pitjj{margin:var(--big-1c1caa8) var(--small-1tsw5sh);}"];
  [@css ".a-94pp3j{padding:var(--small-1d5k27v) 0px;}"];
  [@css ".a-3h97be{border:1px solid var(--alpha-vlpuld);}"];
  [@css ".a-8pyj1z{outline:1px solid var(--alpha-1ftvcb4);}"];
  [@css ".a-3h007w55u{border-bottom:0px solid var(--alpha-1y5ijmp);}"];
  [@css ".a-eckyy8{width:var(--width-18c1xss);}"];
  [@css ".a-88nebp{max-width:var(--max-1yrxnfd);}"];
  [@css ".a-6lvooa{height:var(--height-1n9troi);}"];
  [@css ".a-3nd0d4{border-radius:var(--border-8yt3ey);}"];
  [@css ".a-6500wu0qi{font-size:var(--font-10lt5gt);}"];
  [@css ".a-65001szir{font-family:var(--mono-e5legw);}"];
  [@css ".a-658jkfd7x{line-height:var(--lh-uai7u3);}"];
  [@css ".a-ekfgx3{z-index:var(--zLevel-vut76j);}"];
  [@css ".a-71002egw0{left:var(--left-1s2hr3z);}"];
  [@css ".a-cf0018kmj{text-decoration-color:var(--decorationColor-seqvqc);}"];
  [@css ".a-39008o5gx{background-image:var(--wat-tevp0f);}"];
  [@css
    ".a-7y074nm6u{-webkit-mask-image:var(--externalImageUrl-75mq6y);mask-image:var(--externalImageUrl-75mq6y);}"
  ];
  [@css
    ".a-cy50p4{text-shadow:var(--h-ulszyz) var(--v-1rucg2y) var(--blur-n5ddfa) var(--color-1jwocqc);}"
  ];
  [@css ".a-4e2giy{color:var(--blue-uz6p3q);}"];
  [@css
    ".a-40ihb6{box-shadow:var(--h-1cxj5q6) var(--v-1p9ysn0) var(--blur-1rsz8mu) var(--spread-1icix4c) var(--color-1eos5hw);}"
  ];
  [@css
    ".a-40s4zv{box-shadow:10px 10px 0px var(--spread-wd13u2) var(--color-3xubcm);}"
  ];
  [@css ".a-40l5g5{box-shadow:var(--elevation-jhhehb);}"];
  [@css ".a-40keqi{box-shadow:none;}"];
  [@css ".a-cwbaal{text-overflow:var(--clip-1go0jdu);}"];
  [@css ".a-dm0048edy{transition-duration:500ms;}"];
  [@css ".a-dm0046hu1{transition-duration:var(--duration-i5tvzn);}"];
  [@css
    ".a-2z03ken0s{-webkit-animation-play-state:var(--state-1yj1w6c);animation-play-state:var(--state-1yj1w6c);}"
  ];
  [@css
    ".a-2z03kbnre{-webkit-animation-play-state:paused;animation-play-state:paused;}"
  ];
  [@css
    ".a-6f001nyl9{-webkit-column-gap:var(--px30-cc29mn);column-gap:var(--px30-cc29mn);}"
  ];
  [@css ".a-2k9yln{-webkit-text-fill-color:var(--red-w69rb9);}"];
  module Size = {
    let big = `px(24);
    let small = `px(8);
    let px30 = `px(30);
