...
module CssJs = {
  include Css.Shared.Css_AtomicTypes;
  include Css.Shared.Css_Js_Core;
  include Css.Shared.Css_Colors;
};
...
let runner = tests =>
  List.map(
    item => {
      let (_input, input, _expected, expected) = item;
      test_case(
