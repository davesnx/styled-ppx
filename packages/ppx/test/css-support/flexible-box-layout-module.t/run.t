This test ensures the ppx generates the correct output against styled-ppx.native
If this test fail means that the module is not in sync with the ppx

  $ cat > dune-project << EOF
  > (lang dune 3.10)
  > EOF

  $ cat > dune << EOF
  > (executable
  >  (name input)
  >  (libraries styled-ppx.native)
  >  (preprocess (pps styled-ppx)))
  > EOF

  $ dune build

  $ dune describe pp ./input.re | sed '1,/^];$/d'
  [@css "@property --flex1-3keqbf{syntax:\"*\";inherits:false;}"];
  [@css "@property --value-bdy3z7{syntax:\"*\";inherits:false;}"];
  [@css "@property --value2-1p309rg{syntax:\"*\";inherits:false;}"];
  [@css "@property --value-1g8t0wn{syntax:\"*\";inherits:false;}"];
  [@css "@property --value2-dmenma{syntax:\"*\";inherits:false;}"];
  [@css "@property --min-e3h4r0{syntax:\"*\";inherits:false;}"];
  [@css "@property --value-1g7nd20_1{syntax:\"*\";inherits:false;}"];
  [@css "@property --value-fet175_2{syntax:\"*\";inherits:false;}"];
  [@css ".a-1crvgc7{align-content:flex-start;}"];
  [@css ".a-4zx96o{align-content:flex-end;}"];
  [@css ".a-1c5t6ef{align-content:space-between;}"];
  [@css ".a-1okulzr{align-content:space-around;}"];
  [@css ".a-1jfvg9w{align-items:flex-start;}"];
  [@css ".a-y1kcm0{align-items:flex-end;}"];
  [@css ".a-1bf7e4w{align-self:flex-start;}"];
  [@css ".a-56sg73{align-self:flex-end;}"];
  [@css ".a-k008qs{display:flex;}"];
  [@css ".a-vxcmzt{display:inline-flex;}"];
  [@css ".a-me3p27{-webkit-flex:none;-ms-flex:none;flex:none;}"];
  [@css ".a-yrit56{-webkit-flex:5 7 10%;-ms-flex:5 7 10%;flex:5 7 10%;}"];
  [@css ".a-kzfr2u{-webkit-flex:2;-ms-flex:2;flex:2;}"];
  [@css ".a-p6wv0x{-webkit-flex:10em;-ms-flex:10em;flex:10em;}"];
  [@css ".a-t6vgg1{-webkit-flex:30%;-ms-flex:30%;flex:30%;}"];
  [@css
    ".a-1draax7{-webkit-flex:min-content;-ms-flex:min-content;flex:min-content;}"
  ];
  [@css ".a-ilthbz{-webkit-flex:1 30px;-ms-flex:1 30px;flex:1 30px;}"];
  [@css ".a-6hx0uu{-webkit-flex:2 2;-ms-flex:2 2;flex:2 2;}"];
  [@css ".a-1rr8a55{-webkit-flex:2 2 10%;-ms-flex:2 2 10%;flex:2 2 10%;}"];
  [@css
    ".in-1x4f3wz{-webkit-flex:var(--flex1-3keqbf);-ms-flex:var(--flex1-3keqbf);flex:var(--flex1-3keqbf);}"
  ];
  [@css
    ".in-19g0la7{-webkit-flex:var(--value-bdy3z7) var(--value2-1p309rg);-ms-flex:var(--value-bdy3z7) var(--value2-1p309rg);flex:var(--value-bdy3z7) var(--value2-1p309rg);}"
  ];
  [@css
    ".in-djecpy{-webkit-flex:var(--value-1g8t0wn) var(--value2-dmenma) var(--min-e3h4r0);-ms-flex:var(--value-1g8t0wn) var(--value2-dmenma) var(--min-e3h4r0);flex:var(--value-1g8t0wn) var(--value2-dmenma) var(--min-e3h4r0);}"
  ];
  [@css
    ".in-1f5dbv2{-webkit-flex:var(--value-1g7nd20_1) var(--value-fet175_2);-ms-flex:var(--value-1g7nd20_1) var(--value-fet175_2);flex:var(--value-1g7nd20_1) var(--value-fet175_2);}"
  ];
  [@css ".a-1vu4n70{flex-basis:auto;}"];
  [@css ".a-1ibwkcj{flex-basis:content;}"];
  [@css ".a-71cjcr{flex-basis:1px;}"];
  [@css
    ".a-1l0z8uk{-webkit-flex-direction:row;-ms-flex-direction:row;flex-direction:row;}"
  ];
  [@css
    ".a-8kj89b{-webkit-flex-direction:row-reverse;-ms-flex-direction:row-reverse;flex-direction:row-reverse;}"
  ];
  [@css
    ".a-cgq59l{-webkit-flex-direction:column;-ms-flex-direction:column;flex-direction:column;}"
  ];
  [@css
    ".a-qpiomr{-webkit-flex-direction:column-reverse;-ms-flex-direction:column-reverse;flex-direction:column-reverse;}"
  ];
  [@css ".a-19a80vn{flex-flow:row;}"];
  [@css ".a-o5a89u{flex-flow:row-reverse;}"];
  [@css ".a-14pgqdn{flex-flow:column;}"];
  [@css ".a-1tp8tu8{flex-flow:column-reverse;}"];
  [@css ".a-3hl61a{flex-flow:wrap;}"];
  [@css ".a-3myvs8{flex-flow:wrap-reverse;}"];
  [@css ".a-1tshqlk{flex-flow:row wrap;}"];
  [@css ".a-1a1zoft{flex-flow:row-reverse nowrap;}"];
  [@css ".a-1bnl4v1{flex-flow:column wrap;}"];
  [@css ".a-1n8o660{flex-flow:column-reverse wrap-reverse;}"];
  [@css ".a-2uchni{flex-grow:0;}"];
  [@css ".a-otv6bo{flex-grow:5;}"];
  [@css ".a-6voltb{flex-shrink:1;}"];
  [@css ".a-1b75a5q{flex-shrink:10;}"];
  [@css ".a-1n0sxg9{flex-wrap:nowrap;}"];
  [@css ".a-1jkp9i7{flex-wrap:wrap;}"];
  [@css ".a-hp4whp{flex-wrap:wrap-reverse;}"];
  [@css ".a-1fn0841{justify-content:flex-start;}"];
  [@css ".a-1a9getn{justify-content:flex-end;}"];
  [@css ".a-x4dmss{justify-content:space-between;}"];
  [@css ".a-kp4oss{justify-content:space-around;}"];
  [@css ".a-127g5zj{min-height:auto;}"];
  [@css ".a-i9682b{min-width:auto;}"];
  [@css ".a-12zvlbw{order:0;}"];
  [@css ".a-ali80x{order:1;}"];
  module X = {
    let value = `num(1.);
    let value2 = `num(1.);
    let flex1 = `num(1.);
    let min = `px(500);
  };
  
  CSS.make("a-1crvgc7", []);
  CSS.make("a-4zx96o", []);
  CSS.make("a-1c5t6ef", []);
  CSS.make("a-1okulzr", []);
  CSS.make("a-1jfvg9w", []);
  CSS.make("a-y1kcm0", []);
  CSS.make("a-1bf7e4w", []);
  CSS.make("a-56sg73", []);
  CSS.make("a-k008qs", []);
  CSS.make("a-vxcmzt", []);
  CSS.make("a-me3p27", []);
  CSS.make("a-yrit56", []);
  CSS.make("a-kzfr2u", []);
  CSS.make("a-p6wv0x", []);
  CSS.make("a-t6vgg1", []);
  CSS.make("a-1draax7", []);
  CSS.make("a-ilthbz", []);
  CSS.make("a-6hx0uu", []);
  CSS.make("a-1rr8a55", []);
  CSS.make(
    "in-1x4f3wz",
    [("--flex1-3keqbf", CSS.Types.Flex.toString(X.flex1))],
  );
  CSS.make(
    "in-19g0la7",
    [
      ("--value-bdy3z7", CSS.Types.FlexGrow.toString(X.value)),
      ("--value2-1p309rg", CSS.Types.FlexShrink.toString(X.value2)),
    ],
  );
  CSS.make(
    "in-djecpy",
    [
      ("--value-1g8t0wn", CSS.Types.FlexGrow.toString(X.value)),
      ("--value2-dmenma", CSS.Types.FlexShrink.toString(X.value2)),
      ("--min-e3h4r0", CSS.Types.FlexBasis.toString(X.min)),
    ],
  );
  CSS.make(
    "in-1f5dbv2",
    [
      ("--value-1g7nd20_1", CSS.Types.FlexGrow.toString(X.value)),
      ("--value-fet175_2", CSS.Types.FlexShrink.toString(X.value)),
    ],
  );
  CSS.make("a-1vu4n70", []);
  CSS.make("a-1ibwkcj", []);
  CSS.make("a-71cjcr", []);
  CSS.make("a-1l0z8uk", []);
  CSS.make("a-8kj89b", []);
  CSS.make("a-cgq59l", []);
  CSS.make("a-qpiomr", []);
  CSS.make("a-19a80vn", []);
  CSS.make("a-o5a89u", []);
  CSS.make("a-14pgqdn", []);
  CSS.make("a-1tp8tu8", []);
  CSS.make("a-3hl61a", []);
  CSS.make("a-3myvs8", []);
  CSS.make("a-1tshqlk", []);
  CSS.make("a-1a1zoft", []);
  CSS.make("a-1bnl4v1", []);
  CSS.make("a-1n8o660", []);
  CSS.make("a-2uchni", []);
  CSS.make("a-otv6bo", []);
  CSS.make("a-6voltb", []);
  CSS.make("a-1b75a5q", []);
  CSS.make("a-1n0sxg9", []);
  CSS.make("a-1jkp9i7", []);
  CSS.make("a-hp4whp", []);
  CSS.make("a-1fn0841", []);
  CSS.make("a-1a9getn", []);
  CSS.make("a-x4dmss", []);
  CSS.make("a-kp4oss", []);
  CSS.make("a-127g5zj", []);
  CSS.make("a-i9682b", []);
  CSS.make("a-12zvlbw", []);
  CSS.make("a-ali80x", []);
