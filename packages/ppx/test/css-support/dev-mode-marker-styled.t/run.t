When the PPX is invoked with --dev, every [%styled.<tag>] component gets a
leading `label:<ModuleName>` marker class too, same as [%css] bindings -
covers both a static payload (Box) and a dynamic labeled-argument payload
(Button).
  ...
  module Box = {
    ...
    let styles = CSS.make("label:Box _id_zugc0d _a_4ekvmb", []);
  ...
  module Button = {
    ...
    external innerRefGet: makeProps => option(ReactDOM.domRef) = "innerRef";
    [@mel.get] external as_Get: makeProps => option(string) = "as";
    [@mel.get] external colorGet: makeProps => CSS.Types.Color.t = "color";
    external deleteProp: (Js.t({..}), string) => bool =
      "Reflect.deleteProperty";
    ...
    let styles = (~color: CSS.Types.Color.t, _) =>
      CSS.make(
        "label:Button _id_1qldrk3 _a_4esjgz",
        [("--color-1a279q8", CSS.Types.Color.toString(color))],
      );
    let make = (props: makeProps) => {
      let className =
        fst(styles(~color=colorGet(props), ()))
        ++ getOrEmpty(classNameGet(props))
      and style = snd(styles(~color=colorGet(props), ()));
      let stylesObject =
        makeStylesObject(~className, ~style, ~ref=innerRefGet(props));
      let newProps = assign2(Js.Obj.empty(), props, stylesObject);
