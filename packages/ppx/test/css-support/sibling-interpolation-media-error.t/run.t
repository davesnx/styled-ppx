...
nested inside an at-rule (the value interpolation is still scoped inline on `&`).
  ...
  Error: Cannot interpolate into the value of `width` under `& + td`: the selector targets an element outside `&`'s subtree (via a sibling combinator `+`/`~`). Static extraction passes interpolations as a custom property set inline on `&`, which only `&` and its descendants inherit, so a sibling can't read it and the declaration would be dropped. Instead, target `&` or a descendant, or write a literal value or a globally-inherited theme `var(--...)` directly.
  [1]
