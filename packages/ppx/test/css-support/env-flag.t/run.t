...
keeps readable CSS (the default), `production` equals --minify (minified
CSS). Class names are `css-<hash(content)>` in both modes - --minify only
strips CSS whitespace, it never changes a class name - and the binding
label passed to CSS.make is the same in both modes too.

  $ refmt --parse re --print ml input.re > input.ml

With --env development, no [@@@css.config] is emitted (absence means
development).
