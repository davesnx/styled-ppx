module Option: {let get: (option('a), 'a) => 'a;};
...
let raw_literal: (~loc: Location.t, string) => Parsetree.expression;
let render_unsafe: (~loc: Location.t, string, string) => Parsetree.expression;
let string_to_const: (~loc: Location.t, string) => Parsetree.expression;
let render_html_color: (~loc: Location.t, string) => Parsetree.expression;
let list_to_expr:
...
let group_params:
  ...
  list(
    (
      list(
        (Css_types.Component_value.t, Location.t)
      ),
    Location.t
    )
  );
...
let render_dimension:
  (~loc: Location.t, string, string, Parsetree.constant) =>
...
let render_declaration:
  (Css_types.Declaration.t, Location.t, list((string, string))) =>
  Parsetree.expression;
let render_declarations:
  (
    list(Css_types.Declaration_list.kind),
    option(list((string, string)))
  ) =>
  list(Parsetree.expression);
let render_declaration_list:
  (Css_types.Declaration_list.t, option(list((string, string)))) =>
  Parsetree.expression;
